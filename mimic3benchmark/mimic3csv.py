...
def read_events_table_by_row(mimic3_path, table):
    ...
    for i, row in enumerate(reader):
        if 'ICUSTAY_ID' not in row:#ICUSTAY_ID：key
            row['ICUSTAY_ID'] = ''
...
def filter_diagnoses_on_stays(diagnoses, stays):
    #drop_duplicates()#data中一行元素全部相同时才去除
...
def read_events_table_and_break_up_by_subject(mimic3_path, table, output_path, items_to_keep=None, subjects_to_keep=None, verbose=1):
    ...
    class DataStats(object):
        def __init__(self):
            self.curr_subject_id = ''#当前病人ID
            self.last_write_no = 0#上次写的一行的行号
            self.last_write_nb_rows = 0#总共多少行
            self.last_write_subject_id = ''#上一次的病人ID
            self.curr_obs = []
    ...
    def write_current_observations():
        ...
        '''
        以病人ID subject_id为命名，每人生成一个文件，包含events.csv
        events.csv:包含病人的所有医疗事件
        '''
        try:
            os.makedirs(dn)
        except:
        ...
        if not os.path.exists(fn) or not os.path.isfile(fn):
            ...
            f.close()
        '''
        fieldnames指定字典的key值，如果reader里没有指定那么默认第一行的元素，在writer里一定要指定这个。
        quoting=csv.QUOTE_MINIMAL:引用方式，默认是双引号"
        '''
