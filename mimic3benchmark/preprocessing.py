...
def make_phenotype_label_matrix(phenotypes, stays=None):
    ...
    #空值以0填充，sort_index(axis=0).sort_index(axis=1)，先按行索引（行名）排序，在按列索引排序
...
def read_itemid_to_variable_map(fn, variable_column='LEVEL2'):
    ...
    #set_index('ITEMID'):设置单索引,以ITEMID列
...
def map_itemids_to_variables(events, var_map):
    ...
    左侧DataFrame做连接键的列名为ITEMID，右侧直接使用行索引做连接键名
    '''
...
# Respiratory rate
# Mean blood pressure
#episode{}_timeseries中的列 = clean_fns + ['Hours','Glascow...(4)','Heart Rate','Mean blood pressure','Respiratory rate']
clean_fns = {
