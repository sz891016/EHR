...
def process_partition(args, partition, eps=1e-6, n_hours=48):
    ...
    #元素为元祖(病人ID_时序episode,Mortality),标签元祖
    xy_pairs = []
    patients = list(filter(str.isdigit, os.listdir(os.path.join(args.root_path, partition))))
    for (patient_index, patient) in enumerate(patients):
        patient_folder = os.path.join(args.root_path, partition, patient)
        #筛选出以病人ID命名的文件夹下的包含有timeseries的文件夹
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                ...
                # empty label file(label_df取的是episode.csv)
                ...
                #ts_file是取得episode{#}timeseries.csv
                ...
                with open(os.path.join(output_dir, output_ts_filename), "w") as outfile:
                    outfile.write(header)#列名
                    for line in ts_lines:
                        outfile.write(line)
                   
                #在xy_pairs列表末尾添加新的元素，元素类型为元祖()，打标签，给定死亡与否
