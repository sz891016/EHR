...
parser.add_argument('--data', type=str, help='Path to the data of in-hospital mortality task',
                    default=os.path.join(os.path.dirname(__file__), '../../data/combin-in-hospital-mortality/'))
...
val_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data, 'train'),
                                       listfile=os.path.join(args.data, 'eicuval_listfile.csv'),
                                       period_length=48.0)
...
if args.mode == 'train':
    ...
    tensorboard = TensorBoard(os.path.join(args.output_dir, 'tb_log'), histogram_freq=1,write_graph=True)
    ...
    history=model.fit(x=train_raw[0],
              ...
              verbose=1,
              batch_size=args.batch_size)
    plot_model(model, to_file='lstm_modeltrain.png')
